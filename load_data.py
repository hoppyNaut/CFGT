...
def load_ontonotes4ner(path,char_embedding_path=None,bigram_embedding_path=None,index_token=True,train_clip=False,
    ...
    dev_path = os.path.join(path,'dev.char.bmes{}'.format('_clip' if train_clip else ''))
    ...
    print(datasets.keys())
    print("len(datasets['dev']",len(datasets['dev']))
...
@cache_results(_cache_fp='cache/addressNER_uni+bi', _refresh=False)
def load_address_ner(path,unigram_embedding_path=None,bigram_embedding_path=None,index_token=True,
                   char_min_freq=1,bigram_min_freq=1,only_train_min_freq=0,char_word_dropout=0.01):
    from fastNLP.io.loader import ConllLoader
    from utils import get_bigrams
    ...
    train_path = os.path.join(path,'train_format.txt')
    dev_path = os.path.join(path, 'dev_format.txt')
    test_path = os.path.join(path, 'test_format.txt')
