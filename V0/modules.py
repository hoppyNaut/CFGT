...
class Four_Pos_Fusion_Embedding(nn.Module):
    ...
    def scaling_csl(self, a, b, c, d, w, scalingtype):
        # a b c d 可能为0
        if not isinstance(scalingtype, int):
            raise ValueError("wrong type, got {}.".format(type(scalingtype)))
        if scalingtype > 6 or scalingtype < 1:
            raise ValueError("out of range, got {}.".format(scalingtype))
        if scalingtype == 1:
            # 规则放缩
            if a != 0:
                a = (a / abs(a)) * 0.5
            ...
            # x * (2 * arccot(-w)) / pi
            if w != 0:
                a = a * (2 * math.atan(-1 / w) / math.pi)
                b = b * (2 * math.atan(-1 / w) / math.pi)
                c = c * (2 * math.atan(-1 / w) / math.pi)
                d = d * (2 * math.atan(-1 / w) / math.pi)

        # print("encoding change, using {}".format(scalingtype))
        return a, b, c, d
    ...
    def forward(self,pos_s,pos_e):
        ...
        # 在相应位置添加如下7-22行代码
        # 放缩开始
        list_ss = pos_ss.cpu().numpy().tolist()
        list_se = pos_se.cpu().numpy().tolist()
        list_es = pos_es.cpu().numpy().tolist()
        list_ee = pos_ee.cpu().numpy().tolist()

        batch_size = pos_ss.size(0)
        seq_len = pos_ss.size(1)
        for i in range(batch_size):
            for j in range(seq_len):
                for k in range(seq_len):
                    w = list_ss[i][j][k] * list_ee[i][j][k]
                    if w <= 0:
                        # 放缩方案：1-5
                        # print("list放缩前：",list_ss[i][j][k], list_se[i][j][k], list_es[i][j][k], list_ee[i][j][k])
                        list_ss[i][j][k], list_se[i][j][k], list_es[i][j][k], list_ee[i][j][k] = self.scaling_csl(
                            list_ss[i][j][k], list_se[i][j][k], list_es[i][j][k], list_ee[i][j][k], w, 2)
                        # print("list放缩后：", list_ss[i][j][k], list_se[i][j][k], list_es[i][j][k], list_ee[i][j][k])
                        # print("tensor放缩后：",pos_ss[i][j][k],pos_se[i][j][k],pos_es[i][j][k],pos_ee[i][j][k])

        pos_ss = torch.tensor(list_ss, device='cpu').type(torch.long)
        pos_se = torch.tensor(list_se, device='cpu').type(torch.long)
        pos_es = torch.tensor(list_es, device='cpu').type(torch.long)
        pos_ee = torch.tensor(list_ee, device='cpu').type(torch.long)
...
class Transformer_Encoder(nn.Module):
    ...
    def expandLexLattice(self,seq_len,lex_num,pos_s,pos_e):
        batch_size = pos_s.size(0)
        expandPos_s = pos_s.clone().detach()
        expandPos_e = pos_e.clone().detach()
        for batch in range(batch_size):
            cur_seq_len = seq_len[batch]
            cur_lex_num = lex_num[batch]
            for i in range(cur_seq_len,cur_seq_len + cur_lex_num):
                if expandPos_s[batch,i] > 0:
                    expandPos_s[batch,i] = expandPos_s[batch,i] - 1
                if expandPos_e[batch,i] < cur_seq_len - 1:
                    expandPos_e[batch,i] = expandPos_e[batch,i] + 1
        return expandPos_s,expandPos_e
    ...
    def forward(self, inp, seq_len,lex_num=0,pos_s=None,pos_e=None,print_=False):
        ...
        if self.relative_position:
            if self.four_pos_fusion_shared and self.lattice:
                # rel_pos_embedding = self.four_pos_fusion_embedding(pos_s, pos_e)
                expandPos_s, expandPos_e = self.expandLexLattice(seq_len, lex_num, pos_s, pos_e)

                # rel_pos_embedding = self.four_pos_fusion_embedding(expandPos_s, expandPos_e)
                # rel_pos_embedding = self.four_pos_fusion_embedding(pos_s, pos_e)
                # 融合方法一 先计算相对位置嵌入，再融合
                rel_pos_embedding_1 = self.four_pos_fusion_embedding(pos_s, pos_e)
                rel_pos_embedding_2 = self.four_pos_fusion_embedding(expandPos_s, expandPos_e)
                # print(rel_pos_embedding_1.shape, rel_pos_embedding_2.shape)
                # rel_pos_embedding = self.fusion_layer(torch.cat((rel_pos_embedding_1, rel_pos_embedding_2), -1))
                # rel_pos_embedding = rel_pos_embedding_1 + rel_pos_embedding_2
                # print(torch.cat((rel_pos_embedding_1, rel_pos_embedding_2),-1).shape)
            else:
        ...
        for i in range(self.num_layers):
            ...
            output_1 = now_layer(output_1, seq_len, lex_num=lex_num, pos_s=pos_s, pos_e=pos_e,
                               rel_pos_embedding=rel_pos_embedding_1, print_=print_)
            output_2 = now_layer(output_2, seq_len, lex_num=lex_num, pos_s=expandPos_s, pos_e=expandPos_e,
                               rel_pos_embedding=rel_pos_embedding_2, print_=print_)
        output = self.fusion_layer(torch.cat((output_1, output_2), -1))
        output = self.layer_preprocess(output)
