...
parser.add_argument('--lexicon_name',default='yj',choices=['lk','yj','song'])
...
parser.add_argument('--dataset', default='ontonotes', help='weibo|resume|ontonotes|msra|address')
...
elif args.dataset == 'address':
    datasets, vocabs, embeddings = load_address_ner(address_ner_path, yangjie_rich_pretrain_unigram_path,
                                                    yangjie_rich_pretrain_bigram_path,
                                                    _refresh=refresh_data, index_token=False,
                                                    _cache_fp=raw_dataset_cache_name,
                                                    char_min_freq=args.char_min_freq,
...
elif args.lexicon_name == 'song':
    yangjie_rich_pretrain_word_path = song_word_path
...
# print('dev:{}'.format(len(datasets['dev'])))
    ...
    for i in range(len(v)):
        ...
        if k == 'train':
            ...
            if v[i]['seq_len'] >200:
                ...
                print('-------------------------------------------------')
                print(''.join(list(map(lambda x:vocabs['label'].to_word(x),v[i]['target']))))
            else:
